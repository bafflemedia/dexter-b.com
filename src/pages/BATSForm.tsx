...
import { 
  Camera, UploadCloud, Settings, CheckCircle, Zap, Trash2, Cpu,
  DollarSign, User, MapPin, ShoppingBag, FolderKanban,
  ShieldAlert, FileText, UserCheck, Wrench, PackageOpen, Link as LinkIcon,
  PlusCircle, Search, Receipt, AlertTriangle, Video, CheckSquare, ArrowLeft
} from 'lucide-react';
...
export default function BATSForm({ mode = 'new' }: BATSFormProps) {
  const { batId } = useParams();
  const location = useLocation();
  const sourceAsset = (location.state as { asset?: BatsAsset } | null)?.asset;
  ...
  const [asset, setAsset] = useState<AssetData>({
    ...
    categoryPageId: sourceAsset?.categoryIds[0] || '',
    locationPageId: sourceAsset?.locationIds[0] || '',
    projectPageIds: sourceAsset?.projectIds || [],
    index: sourceAsset?.index?.toString() || '',
    assetClass: sourceAsset?.assetClass || 'Expensed (Section 179)',
    status: sourceAsset?.status || 'Available',
    unitPrice: sourceAsset?.unitPrice?.toString() || '',
    serialNumber: sourceAsset?.serialNumber || '',
    manufacturer: sourceAsset?.manufacturer || '',
    warrantyExpiration: sourceAsset?.warrantyExpiration || '',
    powerDrawWatts: sourceAsset?.powerDrawWatts?.toString() || '',
    notes: sourceAsset?.notes || '',
    aiFieldNote: sourceAsset?.aiFieldNote || '',
    primaryUser: sourceAsset?.primaryUser || 'DexterB',
    syncStatus: sourceAsset?.syncStatus || 'Draft',
  ...
  const submitToNotion = async () => {
    ...
    try {
      if (config.mode === 'mock') {
        ...
        const response = await fetch(isEditing && batId ? `${config.bridgeUrl}/${batId}` : config.bridgeUrl, { 
        ...
        if (response.ok) {
          ...
          const savedPageId = data.page_id || data.id || batId || 'saved-page';
          setVaultData({ 
            baffleId: `NODE-${savedPageId.slice(-4).toUpperCase()}`, 
            batsUrl: `https://notion.so/${savedPageId.replace(/-/g, '')}` 
          });
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans p-4 pb-44 selection:bg-amber-500/30">
      ...
      <header className="flex justify-between items-center mb-6 border-b border-slate-800 pb-4">
        <div>
          <Link to="/bats" className="mb-3 inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 hover:text-amber-500">
            <ArrowLeft size={14} />
            Asset Register
          </Link>
          <div className="flex items-center gap-2">
            <Zap className="text-amber-500 fill-amber-500" size={18} />
            <h1 className="text-xl font-black tracking-tighter italic uppercase text-slate-100">
              {isEditing ? 'Edit BATS Asset' : 'New BATS Asset'}
            </h1>
          </div>
